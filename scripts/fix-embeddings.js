...
if (!SUPABASE_URL || !SUPABASE_KEY || !OPENAI_API_KEY) {
  ...
  console.log(
    "KEY:",
    SUPABASE_KEY ? "OK (" + SUPABASE_KEY.slice(0, 5) + "...)" : "MISSING"
  );
...
async function main() {
  ...
  if (error) {
    ...
    console.error(
      "   Hint: Verifica que SUPABASE_SERVICE_ROLE_KEY sea correcta en .env"
    );
    return;
  }
  ...
  while (hasMore) {
    ...
    const updates = stories.map(async (story) => {
      ...
      if (vector) {
        ...
        if (updateError)
          console.error(
            `❌ Falló update ID ${story.id}: ${updateError.message}`
          );
